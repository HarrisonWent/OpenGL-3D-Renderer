...
{	
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_DrawCount * sizeof(unsigned int),
...
void Mesh::Draw()
{
	m_transform->SetRotation(vec3(0, m_transform->GetRotation().y + 0.003f, 0));
	//m_transform->SetPosition(vec3(m_transform->GetPosition().x + 0.1f, 0, 0));
...
void Mesh::CalculateTangentsBitangent(Vertex* verticies, unsigned int vertCount, unsigned int* indices, unsigned int numIndices)
{
	
	//Calculate the tangent and bitangent
	for (unsigned int i = 0; i < numIndices; i += 3)
	{
		//Get the vertex for the current indicie
